...
class PaysonCheckout1 extends PaymentModule
{
    ...
    {
        if ($this->context->controller->php_self == 'cart' && isset($this->context->cookie->payson_order_error) && $this->context->cookie->payson_order_error != null) {
            $mess = $this->context->cookie->payson_order_error;
            $this::paysonAddLog('Payson order error message: ' . $mess);
            // Delete old messages
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $invoiceFee = Configuration::get('PAYSONCHECKOUT1_INVOICE_FEE');
        if ($invoiceFee != null && $invoiceFee > 0 && is_numeric($invoiceFee)) {
        ...
        if ((Tools::strtolower($cartCurrency->iso_code) == 'sek' && $orderTotal < 4) || (Tools::strtolower($cartCurrency->iso_code) == 'eur' && $orderTotal < 0.4)) {
             $this::paysonAddLog('Order value is less then 4 SEK/Euro 0.4, will not show Payson payment method in checkout.', 2);
            return;
        }
        
        // If only invoice and amount below 30 dont show Payson
        if ((int) Configuration::get('PAYSONCHECKOUT1_PAYMENT_METHODS') == 5 && $orderTotal < 30) {
            $this::paysonAddLog('Payson is set to use invoice only and order value is less then 30, will not show Payson payment method in checkout.', 2);
            return;
        }
...
//                ),
    ...
    public function getRedirectPaymentOption()
    {
        if ($this->active) {
            ...
            $redirectOption->setCallToActionText($this->l('Pay with Payson Checkout 1.0'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirect', array('payredirect' => 'payson'), true))
                    ->setAdditionalInformation($this->context->smarty->fetch('module:paysoncheckout1/views/templates/front/payment_infos.tpl'));
    ...
    public function redirectToPayson()
    {
        try {
            ...
            $cancelUri = $this->context->link->getPageLink('cart', null, null, array('action' => 'show'));
